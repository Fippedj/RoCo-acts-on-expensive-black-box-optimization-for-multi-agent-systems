...
| G-005 | LTReflect 写入内容 | 论文明确 + 自行设计 | p.4 `S007` 明确前后值与变化；p.10–16 `S011` 指出结构缺失 | ADR-0004 已发布并冻结 `roco-memory-event-v1`；minimize 的 `delta_g=before-after`，失败保留 null 结果，候选代码不复制入 memory | P3a 实现严格 schema、trace 转换器和脱敏测试；设计发布不代表运行时完成 | P3a（当前下一任务） |
| G-006 | LTReflect 存储后端 | 必须自行设计 | p.10–16 `S011` | 每代不可变 JSONL segment + 摘要/checkpoint；commit-last marker 以长度和 SHA-256 验证 | P3a 实现 fsync/原子 rename、孤儿检测及 checkpoint 恢复测试 | ADR-0004；P3a |
| G-007 | LTReflect 检索 | 必须自行设计 | p.10–16 `S011` | 前代已提交、同 benchmark/objective/role 的 `K=5`；默认最近改善 3 条、其他/失败 2 条，不足跨类补位，最终旧到新注入 | P3a 先提供可信已提交事实读取边界；P3b 才实现 3/2 作用域、补位和损坏代检索测试 | ADR-0004；P3b（P3a 后） |
| G-008 | memory-guided mutation prompt | 必须自行设计 | p.4 `S007`、p.10 `S008` 只给机制；`S011` 指出 prompt/检索不完整 | 每个配置化精英按 E/X/I 各一次；输入为任务契约、精英、角色摘要、K 条数值事件，输出单候选严格 JSON；外部文本作为数据编码 | 默认 `elite_count=1` 是工程值；P3b 实现角色 Mock、注入和失败测试 | ADR-0004；P3b（P3a 后） |
...
| G-010 | 上下文截断 | 必须自行设计 | p.10–16 `S011` | ADR-0004 固定保留优先级和删除审计；Mock 用确定性字符/字段预算，真实 provider 必须另加 tokenizer 契约 | 字符预算不等于模型 token；P3b 实现边界和稳定截断测试 | P3b；真实 provider 前再版本化 |
...
| G-025 | trace 到长期事件的边界 | 必须自行设计 | 论文没有日志 schema；Stage 3 trace 明确 generation-local | 纯转换器从 proposal/compare/integrate 派生事件；初始 Critic 不入长期 memory；当代事件提交前不可被历史检索 | 来源关联错误会污染记忆；P3a 需 golden mapping 测试 | ADR-0004；P3a |
| G-026 | 角色摘要 multiplicity | 必须自行设计 | 论文说明 LTReflect 机制但未披露每代调用数 | 每代 E/X/I 各一次；Critic 反馈作为证据但不建独立长期摘要；失败沿用旧/空摘要 | 每代额外 3 个 LLM calls；P3b 才实现预算和回退测试 | ADR-0004；P3b（P3a 后） |
| G-027 | 代级恢复 | 必须自行设计 | 论文未披露 crash consistency | population、账本、随机/provider 游标进入 JSON checkpoint；只从连续有效 commit 恢复，半写代忽略 | 显式 JSON-safe snapshot、连续 commit 扫描和恢复读取属于 P3a | ADR-0004；P3a |
...
摘要、mutation 与恢复协议已经由 `cf1e06b`/ADR-0004 设计冻结并发布，但已发布的运行时
实现仍为 0%。当前下一开发任务是 P3a（schema、trace 转换、segment store、commit-last、
checkpoint 恢复底座）；其后才是 P3b（摘要、K=5 检索、截断和 memory mutation 接入）。
Stage 3 的运行 trace 不能用来宣称这些能力已经完成。
