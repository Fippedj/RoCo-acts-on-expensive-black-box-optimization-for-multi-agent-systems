...
| G-042 | EBBO ledger、canonical JSON、稳定 ID 与旧账本兼容 | 必须自行设计 | ADR-0001/现有 `BudgetLedger` 面向 Stage 2--5 LLM/候选 evaluator；昂贵 oracle acceptance、reservation、成功/失败 evaluation 和 attempt ID 尚无代码契约 | ADR-0008/EBBO spec 只冻结 `ebbo-ledger-v1` 概念分账、accepted attempt 必计和非时间 replay 原则；不复用或修改旧语义 | P9a 关闭证据：决定新 ledger/adapter 边界、canonical JSON 与 seed 派生版本，覆盖 duplicate/timeout/cancel/cost-overrun/recovery，并证明旧 smoke/ledger 完全不变 | P9a 前关闭；当前开放 |
| G-043 | EBBO surrogate 与共享 posterior | 必须自行设计 | RoCo 论文和当前仓库没有可直接复用的 GP/神经 surrogate、核、拟合、校准或第三方 BO 依赖证据 | 所有角色共享同一 committed Observation snapshot/posterior；fit artifact 必须版本化，失败不得静默换模型；具体 family=`unset` | P9a 关闭证据：选择依据、依赖/许可证、输入归一化、超参数、数值失败/fallback、seed 和单元测试；不能声称为论文方法 | P9a 前关闭；当前开放 |
| G-044 | acquisition、候选池生成与 tie-break | 必须自行设计 | EI/UCB/Thompson sampling 都是未来候选；pool size、连续/离散优化器和 pending 处理未由材料决定 | acquisition 只对版本化 posterior/pending 评分并产生有限 immutable pool；角色只能引用 pool entry；具体 acquisition/size/optimizer=`unset` | P9a 关闭最小 baseline 选择与确定性 tie-break；比较多个 acquisition 时分别成 cell。P9c 前另关 pending-aware 规则 | P9a/P9c；当前开放 |
| G-045 | EBBO benchmark、搜索域、reference 与真实 oracle 来源 | 必须自行设计 | 当前 TSP/MKP 协议是启发式代码 evaluator，不是已授权的昂贵黑盒 benchmark；没有 Stage 6 数据、许可证或真实 oracle | P9a 仅允许一个新版本化 deterministic Mock function；具体函数/域/reference 仍 `unset`，不得把 Stage 5 数据自动迁入 | P9a 关闭 Mock engineering contract；P10 另需用户明确授权 benchmark/source/license/version/reference/provider/oracle 与预算 | P9a Mock/P10 real；当前开放 |
| G-046 | 噪声、replicate 与 Observation 聚合 | 必须自行设计 | 论文/仓库未定义 EBBO observation noise、heteroscedasticity、replicate 分配或 latent/observed regret | P9a Mock 固定 `noise.kind=none` 且默认拒绝重复；真实噪声策略、聚合与 replicate request ID 仍开放 | P10 前预注册噪声来源、方差语义、replicate policy、聚合、seed 和 regret 定义；重复接受必须逐 attempt 计费 | P9a 部分关闭/P10；当前真实部分开放 |
| G-047 | 可选约束、不可行点与失败观测建模 | 必须自行设计 | 最小问题允许 `g_i(x)<=0`，但约束 surrogate、可行性 acquisition、failure classifier 和无可行点指标未指定 | 失败 Observation 保持 objective=null；只有成功合法结果进入 objective surrogate；约束/失败模型=`unset` | 关闭证据：constraint schema/evaluator、unknown/missing 规则、可行性模型/acquisition、无可行点报告和失败路径测试 | P9a 最小范围/P9c failure-aware；当前开放 |
| G-048 | 异步 reservation、pending、乱序完成、取消与恢复 | 必须自行设计 | ADR-0001/G-017 未冻结昂贵 oracle 在途结算；现实完成顺序不可由 seed 重建 | ADR-0008 冻结 acceptance 后不可退款、pending 占 reservation、completion sequence 入审计、unknown request 不盲重发；P9a/P9b 串行 | P9c 关闭证据：max concurrency、pending acquisition policy、race/late-result/reconciliation、crash recovery、过量 reservation 防护和记录序列 replay | P9c 前关闭；当前开放 |
| G-049 | evaluation cost 模型与成本感知调度 | 必须自行设计 | 现有 cost 主要是 provider 费用；EBBO 可能使用 USD、秒、设备小时或抽象单位，不能互换 | expected/actual cost 都带 source+unit；不同单位不隐式相加；cost-aware acquisition/scheduler=`unset` | P9a 冻结 Mock unit；P9c/P10 前决定估计误差、actual overrun、换算/多资源约束、cost-to-target 和公平 profile | P9a/P9c/P10；当前开放 |
| G-050 | EBBO 指标、target、重复数与统计结论 | 必须自行设计 | 当前没有 EBBO run 或 E4 工件；reference optimum、cumulative regret 的噪声/失败处理、target 和 repetitions 均未知 | 未来报告 simple regret、条件明确时的 cumulative regret、cost-to-target、失败率和时间；跨 benchmark 不聚合 raw score；沿用 E4 结论闸门 | P10 前冻结 reference/target、seed set、重复数、失败/censoring、interval/多重比较和偏离记录；没有 E4 不作性能/显著性/泛化/优越性结论 | P10 前关闭；当前开放 |
| G-051 | EBBO 角色输出、权限与可选 memory | 必须自行设计 | Stage 3 角色生成/融合启发式代码，Stage 4 memory 绑定旧 trace/mutation；均不能直接作为 EBBO 调度协议 | P9b 角色固定为 global explorer/local exploiter/model critic/resource integrator；Integrator 只选有限 pool，scheduler 唯一 dispatch；默认无 EBBO memory | P9b 关闭角色 schema、越权拒绝、失败降级和消融；若未来接 memory，另定 Observation 来源、检索、预算、隐私与 no-memory 对照 | P9b/后续 memory；当前开放 |

## 当前阻断项

Stage 6 P8 设计由 ADR-0008 和 `paper_spec/ebbo_design.md` 冻结，运行时实现尚未开始。
G-042--G-051 都是有意保留的实现/实验选择：P9a 必须先关闭最小 Mock ledger、surrogate、acquisition、
benchmark 和串行策略所需子项；P9b 再关闭受限角色协议；P9c 才关闭异步/pending/failure-aware
调度；P10 只有在用户明确授权 benchmark、来源、许可证、真实 provider/oracle 和预算后才可关闭真实
实验与统计子项。设计完成不关闭这些 gap，也不构成 E4 或任何性能证据。
