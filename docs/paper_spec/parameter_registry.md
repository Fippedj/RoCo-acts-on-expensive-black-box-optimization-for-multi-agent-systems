...
| `ebbo.oracle.mock_contract` | `ebbo-mock-expensive-oracle-v1`；整数 `[-5,5]`，`(x-2)^2+1`，constraints off，noise none，serial | P9a 已验证 | 工程 Mock，不是外部 benchmark、昂贵真实实验或论文事实；G-045/G-046 P9a 子项关闭 |
| `ebbo.surrogate.family` / `fit_contract` | `ebbo-nearest-observation-surrogate-v1`；最近成功 observation mean + normalized distance uncertainty；cold prior `10.0` | P9a 工程 baseline 已验证 | stdlib-only，无拟合/校准/概率 posterior/fallback；通用或真实选择仍开放；见 G-043 |
| `ebbo.acquisition.id` | `ebbo-lower-confidence-bound-v1`，minimize `mean - beta*uncertainty`，smoke `beta=2.0` | P9a 工程 baseline 已验证 | 不等同论文方法或性能已验证 UCB；EI/TS/其他 acquisition 是未来独立 cell；见 G-044 |
| `ebbo.candidate_pool.size` / `optimizer` / `tie_break` | smoke `4` / seeded finite-domain subset / score ascending then full candidate ID ascending | P9a 已验证 | immutable tuple；提案先计数再按 candidate ID 去重；P9b 角色只能选池内；见 G-044 |
